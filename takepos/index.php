...
function ClickProduct(position, qty = 1) {
	...
	if ($('#prodiv'+position).data('iscat') == 1){
		...
		// Si le produit est déjà dans le panier → incrémenter la quantité de 1
		var existingLine = $('tr.posinvoiceline[data-product-id="'+idproduct+'"]').first();
		if (existingLine.length > 0 && invoiceid !== "" && invoiceid !== "0") {
			var currentQty = parseFloat(existingLine.data('qty')) || 0;
			var lineId = existingLine.attr('id');
			var newQty = currentQty + 1;
			console.log("Product "+idproduct+" already in cart (line "+lineId+"), qty "+currentQty+" -> "+newQty);
			selectedline = lineId;
			$("#poslines").load("invoice.php?action=updateqty&token=<?php echo newToken(); ?>&place="+place+"&idline="+lineId+"&number="+newQty, function() {
				<?php if (getDolGlobalString('TAKEPOS_CUSTOMER_DISPLAY')) {
					echo "CustomerDisplay();";
				}?>
				var sl = lineId;
				selectedline = 0;
				$('tr.posinvoiceline#'+sl).trigger('click');
			});
		} else {
...
/* ===== Fonctions de la barre d'édition inline ===== */

function openInlineEdit(lineId, productName, qty, price, reduction) {
	selectedline = lineId;
	$('#editbar-product-name').text(productName);
	$('#editbar-qty').val(parseFloat(qty) || 0);
	$('#editbar-price').val(parseFloat(price) || 0);
	$('#editbar-reduction').val(parseFloat(reduction) || 0);
	$('#editbar-noselect').hide();
	$('#editbar-form').css('display', 'flex');
}

function resetEditBar() {
	selectedline = 0;
	$('#editbar-form').hide();
	$('#editbar-noselect').show();
}

function editbarQtyDelta(delta) {
	var v = parseFloat($('#editbar-qty').val()) || 0;
	$('#editbar-qty').val(Math.max(0, v + delta));
}

function saveEditBarQty() {
	var qty = parseFloat($('#editbar-qty').val());
	if (isNaN(qty) || !selectedline) return;
	invoiceid = $('#invoiceid').val();
	if (qty <= 0) {
		deleteEditBarLine();
		return;
	...
	var sl = selectedline;
	$("#poslines").load("invoice.php?action=updateqty&token=<?php echo newToken(); ?>&place="+place+"&idline="+sl+"&number="+qty, function() {
		selectedline = 0;
		$('tr.posinvoiceline#'+sl).trigger('click');
	});
}

function saveEditBarPrice() {
	var price = $('#editbar-price').val();
	if (price === '' || !selectedline) return;
	var sl = selectedline;
	$("#poslines").load("invoice.php?action=updateprice&token=<?php echo newToken(); ?>&place="+place+"&idline="+sl+"&number="+price, function() {
		selectedline = 0;
		$('tr.posinvoiceline#'+sl).trigger('click');
	});
}

function saveEditBarReduction() {
	var reduction = $('#editbar-reduction').val();
	if (reduction === '' || !selectedline) return;
	var sl = selectedline;
	$("#poslines").load("invoice.php?action=updatereduction&token=<?php echo newToken(); ?>&place="+place+"&idline="+sl+"&number="+reduction, function() {
		selectedline = 0;
		$('tr.posinvoiceline#'+sl).trigger('click');
	});
}

function deleteEditBarLine() {
...
<div id="ModalNote" class="modal">
	...
	<div class="row1<?php if (!getDolGlobalString('TAKEPOS_HIDE_HEAD_BAR')) {
		...
		<div id="poslines" class="div1" style="width:40%;">
		</div>

		<div class="div2" id="pos-editbar-panel" style="width:20%; display:flex; flex-direction:column; justify-content:center; padding:10px 8px; box-sizing:border-box;">

			<!-- Message quand aucune ligne sélectionnée -->
			<div id="editbar-noselect" style="display:flex; flex:1; flex-direction:column; align-items:center; justify-content:center; color:#aaa; text-align:center; gap:10px; font-size:13px;">
				<i class="fa fa-hand-pointer" style="font-size:2em; opacity:0.35;"></i>
				<span><?php echo $langs->trans("SelectALineToEdit"); ?></span>
			</div>

			<!-- Formulaire édition ligne (visible quand ligne sélectionnée) -->
			<div id="editbar-form" style="display:none; flex-direction:column; gap:10px; width:100%;">

				<!-- Nom produit -->
				<div id="editbar-product-name" style="font-weight:600; font-size:13px; padding:7px 10px; background:var(--colorbacklinepairchecked, #eef2ff); border-radius:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; border:1px solid #dde2f0;"></div>

				<!-- Quantité -->
				<div style="display:flex; align-items:center; gap:6px;">
					<span style="width:65px; font-size:12px; color:#555; flex-shrink:0;"><?php echo $langs->trans("Qty"); ?></span>
					<button type="button" onclick="editbarQtyDelta(-1)" style="width:30px; height:30px; border:1px solid #ccc; border-radius:5px; background:#f5f5f5; font-size:16px; cursor:pointer; flex-shrink:0;">−</button>
					<input type="number" id="editbar-qty" min="0" step="1" style="flex:1; height:30px; text-align:center; font-size:14px; border:1px solid #ccc; border-radius:5px; padding:0 4px; min-width:0;">
					<button type="button" onclick="editbarQtyDelta(1)" style="width:30px; height:30px; border:1px solid #ccc; border-radius:5px; background:#f5f5f5; font-size:16px; cursor:pointer; flex-shrink:0;">+</button>
					<button type="button" onclick="saveEditBarQty()" title="<?php echo $langs->trans("Save"); ?>" style="width:30px; height:30px; background:#5555AA; color:#fff; border:none; border-radius:5px; cursor:pointer; flex-shrink:0;"><i class="fa fa-check"></i></button>
				</div>

				<!-- Prix -->
				<div style="display:flex; align-items:center; gap:6px;">
					<span style="width:65px; font-size:12px; color:#555; flex-shrink:0;"><?php echo $langs->trans("Price"); ?> <?php echo getDolGlobalInt('TAKEPOS_CHANGE_PRICE_HT') ? 'HT' : 'TTC'; ?></span>
					<input type="number" id="editbar-price" min="0" step="0.01" style="flex:1; height:30px; text-align:right; font-size:14px; border:1px solid #ccc; border-radius:5px; padding:0 6px; min-width:0;">
					<button type="button" onclick="saveEditBarPrice()" title="<?php echo $langs->trans("Save"); ?>" style="width:30px; height:30px; background:#5555AA; color:#fff; border:none; border-radius:5px; cursor:pointer; flex-shrink:0;"><i class="fa fa-check"></i></button>
				</div>

				<!-- Remise -->
				<div style="display:flex; align-items:center; gap:6px;">
					<span style="width:65px; font-size:12px; color:#555; flex-shrink:0;"><?php echo $langs->trans("LineDiscountShort"); ?> %</span>
					<input type="number" id="editbar-reduction" min="0" max="100" step="1" style="flex:1; height:30px; text-align:right; font-size:14px; border:1px solid #ccc; border-radius:5px; padding:0 6px; min-width:0;">
					<button type="button" onclick="saveEditBarReduction()" title="<?php echo $langs->trans("Save"); ?>" style="width:30px; height:30px; background:#5555AA; color:#fff; border:none; border-radius:5px; cursor:pointer; flex-shrink:0;"><i class="fa fa-check"></i></button>
				</div>

				<!-- Bouton Supprimer — même largeur que l'input, aligné sous Remise -->
				<div style="display:flex; align-items:center; gap:6px;">
					<span style="width:65px; flex-shrink:0;"></span>
					<button type="button" onclick="deleteEditBarLine()" style="flex:1; height:30px; background:rgb(255,188,185); color:#633; border:none; border-radius:6px; cursor:pointer; font-size:12px; display:flex; align-items:center; justify-content:center; gap:6px; min-width:0;">
						<i class="fa fa-trash"></i> <?php echo $langs->trans("Delete"); ?>
					</button>
