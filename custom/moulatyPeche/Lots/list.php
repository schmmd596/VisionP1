...
$search_etat    = GETPOST('search_etat', 'alpha'); // Ajout pour l'état du lot
$search_source  = GETPOST('search_source', 'alpha'); // Ajout pour le type de source
...
// Filtre pour l'état du lot
if ($search_etat && $search_etat != 'all') {
    if ($search_etat == 'vide') {
        $where[] = " NOT EXISTS (SELECT 1 FROM ".MAIN_DB_PREFIX."pech_lotdet ld2 
                    INNER JOIN ".MAIN_DB_PREFIX."pech_carton c ON c.fk_lotdet = ld2.rowid 
                    WHERE ld2.fk_lot = l.rowid)";
    } else {
        $where[] = " l.rowid IN (" . getLotEtatSubquery($search_etat) . ")";
    }
}

// Filtre pour le type de source
if ($search_source && $search_source != 'all') {
    switch($search_source) {
        case 'directe':
            $where[] = " l.source_type = 1";
            break;
        case 'misenplat':
            $where[] = " l.source_type = 0 AND EXISTS (
                SELECT 1 FROM ".MAIN_DB_PREFIX."pech_lotdet ld 
                WHERE ld.fk_lot = l.rowid 
                AND (ld.fk_misenplat IS NOT NULL AND ld.fk_misenplat > 0)
            )";
            break;
        case 'transfert':
            $where[] = " (l.source_type = 0 AND l.commentaire LIKE '%Créé automatiquement depuis la sortie%') 
                       OR (l.source_type = 0 AND NOT EXISTS (
                           SELECT 1 FROM ".MAIN_DB_PREFIX."pech_lotdet ld 
                           WHERE ld.fk_lot = l.rowid 
                           AND (ld.fk_misenplat IS NOT NULL AND ld.fk_misenplat > 0)
                       ))";
...
// Comptage total
$sql_count = "SELECT COUNT(*) as nb FROM ".MAIN_DB_PREFIX."pech_lot l $sql_where";
...
print '<th>'.$langs->trans("EtatLot").'</th>';
print '<th>'.$langs->trans("TypeSource").'</th>';
...
// État du lot (stock/sorti/partiel/vide)
$etat_options = array(
    'all' => $langs->trans("Tous"),
    'stock' => $langs->trans("EnStock"),
    'sorti' => $langs->trans("Sorti"),
    'partiel' => $langs->trans("Partiel"),
    'vide' => $langs->trans("Vide")
);
print '<td>'.$form->selectarray('search_etat', $etat_options, $search_etat ?: 'all', 0, 0, 0, '', 0, 0, 0, 'class="search-select"').'</td>';

// Type de source
$source_options = array(
    'all' => $langs->trans("Tous"),
    'directe' => $langs->trans("ReceptionDirecte"),
    'misenplat' => $langs->trans("ParMisenplat"),
    'transfert' => $langs->trans("Transfert")
);
print '<td>'.$form->selectarray('search_source', $source_options, $search_source ?: 'all', 0, 0, 0, '', 0, 0, 0, 'class="search-select"').'</td>';
...
if ($resql && $db->num_rows($resql) > 0) {
    ...
    print '<th class="col-source"><i class="fa fa-truck-loading"></i> '.$langs->trans("TypeSource").'</th>';
    ...
    while ($obj = $db->fetch_object($resql)) {
        ...
        $source_type = getLotSourceType($obj->rowid, $obj->source_type, $obj->commentaire);
        ...
        print '<td class="col-source">'.displaySourceTypeBadge($source_type).'</td>';
    ...
    if ($pages > 1) {
        ...
        for ($i = 0; $i < $pages; $i++) {
            ...
            if ($search_etat) $url .= '&search_etat='.urlencode($search_etat);
            if ($search_source) $url .= '&search_source='.urlencode($search_source);
...
/**
 * Fonction pour déterminer le type de source d'un lot
 */
function getLotSourceType($lot_id, $source_type, $commentaire) {
    global $db;
    
    // Si source_type = 1, c'est une réception directe
    if ($source_type == 1) {
        return 'directe';
    }
    
    // Si source_type = 0, vérifier s'il y a des mis en plat
    $sql = "SELECT COUNT(*) as nb_misenplat 
            FROM " . MAIN_DB_PREFIX . "pech_lotdet 
            WHERE fk_lot = " . (int)$lot_id . " 
            AND (fk_misenplat IS NOT NULL AND fk_misenplat > 0)";
    
    $res = $db->query($sql);
    if ($res && ($obj = $db->fetch_object($res))) {
        if ($obj->nb_misenplat > 0) {
            return 'misenplat';
        }
    }
    
    // Vérifier si le commentaire contient "Créé automatiquement depuis la sortie"
    if (!empty($commentaire) && strpos($commentaire, 'Créé automatiquement depuis la sortie') !== false) {
        return 'transfert';
    }
    
    // Par défaut ou pour les transferts
    return 'transfert';
}

/**
 * Fonction pour afficher le badge du type de source
 */
function displaySourceTypeBadge($source_type) {
    switch($source_type) {
        case 'directe':
            return '<span class="badge badge-primary"><i class="fa fa-truck"></i> Directe</span>';
        case 'misenplat':
            return '<span class="badge badge-info"><i class="fa fa-cube"></i> Mis en plat</span>';
        case 'transfert':
            return '<span class="badge badge-warning"><i class="fa fa-exchange-alt"></i> Transfert</span>';
        default:
            return '<span class="badge badge-secondary"><i class="fa fa-question"></i> Inconnu</span>';
    }
}
...
/**
 * Fonction pour la sous-requête d'état du lot
 */
function getLotEtatSubquery($etat) {
    global $db;
    
    $sql = "SELECT DISTINCT ld.fk_lot 
    ...
    switch($etat) {
        case 'stock':
            $sql .= "SUM(CASE WHEN c.statut = 0 THEN 1 ELSE 0 END) = COUNT(c.rowid)";
            break;
        case 'sorti':
            $sql .= "SUM(CASE WHEN c.statut = 1 THEN 1 ELSE 0 END) = COUNT(c.rowid)";
            break;
        case 'partiel':
            $sql .= "SUM(CASE WHEN c.statut = 0 THEN 1 ELSE 0 END) > 0 
                    AND SUM(CASE WHEN c.statut = 1 THEN 1 ELSE 0 END) > 0";
            break;
        default:
            $sql .= "1=1";
    }
