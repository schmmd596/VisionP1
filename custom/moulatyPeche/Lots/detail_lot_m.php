...
// ============================================================================
$monnaieId = getDeviseEntrepot($db, $lot->fk_entrepot);
$monnaiName = getNomMonnaie($db, $monnaieId);
$code_devise = getCodeDeviseFromId($db, $monnaieId);

// Conversions pour affichage
$conversion_valeur_lot = convertFromMRO($db, $valeurLottot, $monnaieId);
$conversion_frais = convertFromMRO($db, $lot->total_frais, $monnaieId);
$conversion_total_facture = convertFromMRO($db, $totalFacture, $monnaieId);
$conversion_total_bon_entree = convertFromMRO($db, $totalBonEntree, $monnaieId);
...
function displayAmountWithConversion($conversion, $show_mro = true) {
    ...
    if ($show_mro && $conversion['original'] > 0) {
        $html .= '<br><small class="text-muted" style="font-style: italic;">(' . price(round($conversion['original'],2)) . ' ' . $conf->currency . ')</small>';
    }
...
if ($code_devise !== 'MRO') {
    ...
    $infos = [
        ...
        // Frais
        ['icon'=>'fa-dollar-sign','label'=>$langs->trans("Frais"),
         'value'=>'<strong>' . displayAmountWithConversion($conversion_frais) . '</strong>'],
        
        // Valeur des lots
        ['icon' => 'fa-dollar-sign','label'=>$langs->trans("ValeurLots"),
         'value'=>'<strong>' . displayAmountWithConversion($conversion_valeur_lot) . '</strong>'],
        
        // Facture fournisseur
        //['icon'=>'fa-file-invoice','label'=>$langs->trans("ValeurFactureFournisseur"),
        // 'value'=>'<strong>' . displayAmountWithConversion($conversion_total_facture) . '</strong>'],
        
        // Bon d'entrée
        //['icon'=>'fa-box-open','label'=>$langs->trans("ValeurBonEntree"),
        // 'value'=>'<strong>' . displayAmountWithConversion($conversion_total_bon_entree) . '</strong>'],
    ...
    if (!empty($lines)) {
        ...
        print '<div>'.price(round($conversion_frais['converted'], 2)).' '.$conversion_frais['currency_code'].'</div>';
        if ($conversion_frais['currency_code'] != 'MRO') {
            print '<small class="text-muted">('.price($lot->total_frais).' '.$conf->currency.')</small>';
        }
        print '</div>';
        print '</div>';
    ...
    if ($lot->statut == 0) {
        ...
        if ($has_locked) {
            ...
            if ($lot->fk_bonentree > 0) {
                print '<a href="doc_bon.php?id='.$lot->fk_bonentree.'" class="detail-button detail-button-primary"><i class="fa fa-eye"></i> Voir Bon</a>';
            }
            ...
            if ($lot->fk_bonentree > 0) {
                print '<a href="doc_bon_m.php?id='.$lot->fk_bonentree.'" class="detail-button detail-button-primary"><i class="fa fa-eye"></i> Voir Bon</a>';
            } else {
                print '<a href="bon_lot_m.php?id_lot='.$id_lot.'&fk_entrepot='.$lot->fk_entrepot.'" class="detail-button detail-button-success"><i class="fa fa-plus-circle"></i> Créer Bon Entrée</a>';
            }
