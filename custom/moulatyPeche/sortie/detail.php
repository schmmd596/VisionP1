...
$sqljhjhjhksjkll = "SELECT ef.monnaie
                ...
                WHERE ef.fk_object = ".((int)$sortie->fk_entrepot_source);;
        ...
        if (!$is_multidevise){
            ...
            header("Location: detail_m.php?id=".$sortie->rowid);
        }else{
...
print '<div class="detail-container">';
    ...
    print '<tr><td class="field-label"><i class="fa fa-coins"></i> '.$langs->trans("Frais").'</td><td class="field-value"><strong>'.price($sortie->total_frais).'</strong></td></tr>';
    ...
    if ($resql_prod && $db->num_rows($resql_prod) > 0) {
        ...
        print '<div class="total-value">'.price(round($total_valeur + $sortie->total_frais, 2)).' '.$conf->currency.'</div>';
        print '</div>';
        print '</div>';
        print '</div>';
    }
    ...
    if ($sortie->statut == 0) {
        ...
        if (!empty($sortie->fk_facture)) {
            ...
            print '<a href="facturer.php?id_sortie='.$sortie->rowid.'" class="detail-button detail-button-warning"><i class="fa fa-file-invoice"></i> '.$langs->trans("Facturer").'</a>';
        }
...
print '</a>';
    ...
    if ($sortie->type == 1 && !empty($sortie->fk_facture_client)) {
        ...
        if ($resFactClient && $db->num_rows($resFactClient) > 0) {
            ...
            print '<td class="text-right"><strong>'.price($fact->total_ht).' '.$conf->currency.'</strong></td>';
            print '<td class="text-right"><strong>'.price($fact->total_ttc).' '.$conf->currency.'</strong></td>';
            print '</tr>';
            print '</tbody>';
            print '</table>';
            print '</div>';
