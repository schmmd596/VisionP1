...
print '<option value="" selected disabled>-- '.$langs->trans("DefaultCurrency").' --</option>';
foreach($currencies as $code => $cur) {
    print '<option value="'.$code.'">'.$cur['label'].' ('.$code.') | '.$langs->trans("Rate").': '.$cur['rate'].'</option>';
}
print '</select>';
print '</div>';
print '</div>';
...
print '<th><i class="fa fa-lightbulb"></i> '.$langs->trans("RecommendedPU").' ('.$conf->currency.')</th>';
print '<th><i class="fa fa-money-bill"></i> '.$langs->trans("UnitPrice").' ('.$conf->currency.')</th>';
print '<th><i class="fa fa-calculator"></i> '.$langs->trans("LineTotal").'</th>';
print '</tr>';
print '</thead>';
print '<tbody>';
...
foreach($produits as $prod) {
    ...
    print '<td class="recommended-price">'.number_format($prod['pu_moyen_carton'], 2).'</td>';
    print '<td><input type="number" step="0.01" name="pu_'.$prod['ids'].'" class="price-input" min="'.number_format($prod['pu_moyen_carton'], 2).'" value="'.number_format($prod['pu_moyen_carton'], 2).'"></td>';
...
document.querySelectorAll('.price-input').forEach(function(input){
    input.addEventListener('input', function(){
        ...
        let nb_carton = parseFloat(row.querySelector('.nbcarton').dataset.poids);
        let total = nb_carton * parseFloat(input.value || 0);
